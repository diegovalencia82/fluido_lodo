      ...
      subroutine tasa_deformacion_epsilon2(mspace,ntype,npairs,pairs,
     +     nfilas,w,dwdx,mxij,mvij,mvxij,epsilon,epsilonq)
...
c     epsilon(6,i) : Tasa de deformación $\varepsilon^{zz}$
     ...
     +     divvdwdx,sumdivg,c23
      double precision epsilonxx,epsilonyy,epsilonzz,epsilonxy,epsilonxz
     +     ,epsilonyz,epsilonq(ntype(1))
...
c      epsilonq = 0.
     ...
     +           mvxij(2,j,i)*dwdx(2,j,i) + mvxij(3,j,i)*dwdx(3,j,i) )
         ...
         epsilonxx = epsilon(1,i)*epsilon(1,i)
         epsilonyy = epsilon(4,i)*epsilon(4,i)
         epsilonzz = epsilon(6,i)*epsilon(6,i)
         epsilonxy = epsilon(2,i)*epsilon(2,i)
         epsilonxz = epsilon(3,i)*epsilon(3,i)
         epsilonyz = epsilon(5,i)*epsilon(5,i)

         epsilonq(i) = epsilonxx + epsilonyy + epsilonzz + 2*epsilonxy +
     +        2*epsilonxz + 2*epsilonyz
