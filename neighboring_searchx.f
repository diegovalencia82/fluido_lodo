...
c     mxiij      : matrix of xi-xj for all fluid particles for each interaction [out]            
      ...
      double precision mspace(25,nmax),mspace_t(25,nmax)
      ...
      double precision x(ntype(1)),z(ntype(1)),r(ntype(1))
      double precision xx,yy,zz,xxj,yyj,zzj,vxx,vyy,vzz,vxxj,vyyj,vzzj
...
c      write(*,*)'Ordena'
      do i = 1,ntype(1)
         x(i) = mspace(2,i)
         z(i) = mspace(4,i)
c         r(i) = sqrt(x(i)*x(i) + z(i)*z(i))
         ind_x(i) = int(mspace(1,i))
c         ind_z(i) = int(mspace(1,i))
      enddo

! Llamar a la subrutina indexx para ordenar por r
      if(itimestep.gt.0)call indexx(ntype(1),x,ind_x)
c      if(itimestep.gt.0)call indexx(ntype(1),z,ind_z)
c      write(*,*)'Sorterd particles=',ntype(1),ntype(1)-ntype(2),ntype(2)
...
c      l = 0
      do k = 1,ntype(1)         !ntype(1) - ntype(2)
         i = ind_x(k)
c         l = ind_z(k)
         if(mspace(12,i).eq.1.)then
c            call calcular_rango(i,ntype(1),window_size,init,ifin)
            nfilas(i) = 0
            do h = 1,ntype(1)   !init,ifin
               j = ind_x(h)
               if(mspace(2,j).gt.mspace(2,i)-rdomain*2.5.and.mspace(2,j)
     +              .lt.mspace(2,i)+rdomain*2.5)then
               if(mspace(4,j).gt.mspace(4,i)-rdomain*2.5.and.mspace(4,j)
     +              .lt.mspace(4,i)+rdomain*2.5)then
...
c         do h=1,nfilas(k)
c            j = ind_x(h)
c            if(pairs(h,k).gt.0)write(*,*)'kh',k,h,pairs(h,k),
c     +           mspace(1,pairs(h,k))
c         enddo
c      enddo
